...
def generate_responses(rolebench_data, model_path):
    ...
    llm_engine = LLM(model=model_path, max_model_len=10000, dtype="bfloat16", tensor_parallel_size=num_gpus, enable_prefix_caching=True, cpu_offload_gb=0, gpu_memory_utilization=0.8, max_num_seqs=100)
    ...
    length = len(prompts_positive)
    outputs = llm_engine.generate(prompts_positive+prompts_negative, sampling_params)
    outputs_positive = outputs[0:length]
    outputs_negative = outputs[length:]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate specific contrastive samples for roleplay data.")
